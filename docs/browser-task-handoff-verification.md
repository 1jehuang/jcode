...
| Public JSON text and metadata agree, with structured `status`, `reason`, `model`, `decision_provider`, `requested_help`, `action_trace`, `final_observation` | Whole-task live assertions plus the live navigation/script helper | Public text equaled metadata. Status was done, model was `typesafe/jev-1.13`, provider/reason were present, help was null on success. Each live step had executed status, confidence, before/after URL evidence and an object result. Final observation contained the required completion text. |
