...
/// Display configuration for TUI performance tuning.
/// Inline definition to avoid dependency on crate::config.
#[derive(Debug, Clone)]
...
pub fn tui_policy() -> TuiPerfPolicy {
    ...
    tui_policy_for(profile(), &display)
}
...
fn detect() -> SystemProfile {
    ...
    let tier = match std::env::var("JCODE_PERFORMANCE_TIER")
        .ok()
        .as_deref() {
        Some("full") => PerformanceTier::Full,
        Some("reduced") => PerformanceTier::Reduced,
        Some("minimal") => PerformanceTier::Minimal,
        _ => auto_tier,
    };
