...
impl AstParser {
    pub fn with_defaults() -> Result<Self, anyhow::Error> { Ok(Self) }

    pub async fn parse(&self, _content: &str, _lang: SupportedLanguage, _path: &str) -> Result<(), anyhow::Error> {
        Ok(())
    }

    pub async fn extract_symbols(&self, _tree: &(), _content: &str, _path: &str, _lang: SupportedLanguage) -> Result<Vec<SymbolInfo>, anyhow::Error> {
        Ok(vec![])
    }
}

#[derive(Debug, Clone, Copy)]
pub enum SupportedLanguage { Rust, TypeScript, Python, Go, }
...
impl CodeAnalyzer {
    pub fn new() -> Result<Self, anyhow::Error> { Ok(Self) }
}

pub trait Analyzable {
    fn analyze_file(&self, path: &Path) -> impl std::future::Future<Output = Result<FileAnalysis, anyhow::Error>> + Send;
}

impl Analyzable for CodeAnalyzer {
    async fn analyze_file(&self, path: &Path) -> Result<FileAnalysis, anyhow::Error> {
        Ok(FileAnalysis {
