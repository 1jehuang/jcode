...
use crate::refactoring::compilation::{CompilationEngine, FixEngine};
...
mod project_scanner {
    use super::*;
    pub async fn scan_project(_workspace: &std::path::Path, _config: &PipelineConfig) -> Result<Vec<ScannedFile>, String> {
        Ok(vec![])
    }
}
...
mod file_analyzer {
    use super::*;
    pub async fn analyze_files(_workspace: &std::path::Path, _files: &[String], _depth: usize) -> Result<Vec<FileAnalysisResult>, String> {
        Ok(vec![])
    }
}
...
impl AstRenamer {
    ...
    async fn rename_ast(&self, _file: &str, _old_symbol: &str, _new_name: &str) -> Result<String, String> {
        Ok(format!("renamed {} -> {}", _old_symbol, _new_name))
    }
}
