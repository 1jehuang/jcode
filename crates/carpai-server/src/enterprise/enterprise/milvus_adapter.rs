...
impl MilvusClient {
    ...
    /// 删除向量
    pub async fn delete_vectors(&self, _ids: &[&str]) -> anyhow::Result<()> {
        if !self.initialized {
            anyhow::bail!("Milvus client not initialized");
        }
        ...
        #[cfg(feature = "milvus")]
        if let Some(client) = &self.client {
            let expr = format!("id in [{}]", _ids.iter().map(|id| format!("\"{}\"", id)).collect::<Vec<_>>().join(","));

            client
                .delete(&self.config.collection_name, &expr)
                .await?;

            info!("Deleted {} vectors from Milvus", _ids.len());
        }
