...
use jcode_auth::rbac::{Role, Permission, PermissionContext};
use serde::{Deserialize, Serialize};
use std::sync::Arc;

use crate::enterprise::auth::{User, UserRole, Organization, OrgPlan, hash_password, JwtManager, JwtClaims};
use crate::enterprise::quota::{QuotaPolicy, UsageTier};
use crate::enterprise::enterprise::EnterpriseServerState;
use crate::enterprise::admin_api::auth_middleware::{auth_middleware, current_user_id, current_org_id};
...
fn login_handler(
    ...
    let claims = JwtClaims::new("user-1".into(), "carpai-enterprise".into(), 24)
        .with_claim("org_id", serde_json::json!("org-1"))
        .with_claim("role", serde_json::json!("org_admin"));
    let token = auth.generate_token(claims).unwrap_or_default();
...
fn list_api_keys_handler(
    State(state): State<Arc<EnterpriseServerState>>,
) -> impl IntoResponse {
    let api_keys = state.api_key_store.read().await;

    let keys: Vec<serde_json::Value> = api_keys.iter().map(|(key, info)| {
        serde_json::json!({
            "key_prefix": &key[..8.min(key.len())],
            "user_id": info.user_id,
            "created_at": info.created_at.to_rfc3339(),
            "last_used": info.last_used.map(|t| t.to_rfc3339()),
...
async fn generate_api_key_handler(
    ...
    if let Some(user) = users.values_mut().next() {
        let api_key = crate::enterprise::auth::generate_api_key(&user.id);
        return (
            StatusCode::CREATED,
            Json(serde_json::json!({
                "api_key": api_key,
...
/// GET /admin/audit
fn get_audit_handler() -> impl IntoResponse {
...
/// POST /admin/models
fn add_model_handler() -> impl IntoResponse {
...
async fn list_roles_handler(
    ...
    let engine = state.jwt_manager.read().await.policy_engine.read().await;
...
le_handler(
    ...
    let mut engine = state.jwt_manager.write().await.policy_engine.write().await;
    ...
    let scope = match req.scope.as_str() {
        "global" => PermissionContext::Global,
        "organization" => PermissionContext::Organization(req.scope_value.unwrap_or_default()),
        "team" => PermissionContext::Team(req.scope_value.unwrap_or_default()),
        "project" => PermissionContext::Project(req.scope_value.unwrap_or_default()),
        _ => PermissionContext::Global,
    };
...
async fn assign_role_handler(
    ...
    state.jwt_manager.read().await.assign_role_to_user(user_id.clone(), req.role_id.clone()).await;
