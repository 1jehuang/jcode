...
use super::{
    AGY_PROFILE_ID, ALL_OPENAI_MODELS, AccountModelAvailabilityState, CHATGPT_WEB_MODEL,
    GROK_BUILD_PROFILE_ID, ModelRoute, MultiProvider, Provider, ProviderRegistry,
...
pub(super) fn multiprovider_model_routes(provider: &MultiProvider) -> Vec<ModelRoute> {
    ...
    // Antigravity CLI (`agy`): an external agent backend, offered like Grok Build.
    if let Some(agy) = ProviderRegistry::new(provider)
        .compatible_profile(AGY_PROFILE_ID)
        .or_else(|| {
            super::external::instantiate_expected_external_provider(super::external::AGY_RUNTIME)
        })
    {
        for mut route in agy.model_routes() {
            route.model = format!("agy:{}", route.model);
            route.provider = "Antigravity (agy)".to_string();
            route.api_method = "agy-cli".to_string();
            if !routes.iter().any(|existing| {
                existing.model == route.model && existing.api_method == route.api_method
            }) {
                routes.push(route);
