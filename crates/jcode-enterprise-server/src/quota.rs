    ...
    pub fn check_quota(
        ...
        match request_type {
            QuotaRequest::ChatCompletion { tokens, model } => {
                // 检查Token配额
                if record.tokens_used + tokens > policy.limits.max_tokens_per_month {
            ...
            QuotaRequest::StartSession => {
                // 检查并发会话
                if record.active_sessions >= policy.limits.max_concurrent_sessions {
                    return Err(QuotaError::ConcurrentSessionExceeded {
                        current: record.active_sessions,
