...
use crate::enterprise::{
    admin_api,
    auth::{User, JwtManager, RbacEngine},
...
pub struct EnterpriseServerState {
    ...
    /// JWT 管理器（来自 crates/jcode-auth）
    pub jwt_manager: Arc<RwLock<JwtManager>>,
    /// RBAC 权限引擎（来自 crates/jcode-auth）
    pub rbac_engine: Arc<RwLock<RbacEngine>>,
    /// 用户存储
    ...
    /// 代码库索引引擎 (Phase 3.1)
    #[cfg(feature = "codebase")]
    pub codebase_engine: Arc<tokio::sync::Mutex<Option<carpai_codebase::CodebaseEngine>>>,
    #[cfg(not(feature = "codebase"))]
    pub codebase_engine: Arc<tokio::sync::Mutex<Option<String>>>,
    /// 启动时间
...
impl EnterpriseServer {
    ...
    pub async fn new(config_path: Option<String>) -> anyhow::Result<Self> {
        ...
        // 启动后台代码库索引进程 (Phase 3.1) — 需要 feature = "codebase"
        #[cfg(feature = "codebase")]
        {
        ...
        let node_hw = jcode_unified_scheduler::NodeHardwareInfo {
            ...
            memory_gb: total_mem,
            tflops_fp16: 0.01 * cpu_cores as f64,
    ...
    pub async fn serve(&self) -> anyhow::Result<()> {
        ...
        // 5. 启动 gRPC 服务（CarpVoid / IDE 客户端接入）
        let grpc_addr = format!("{}:{}", config.server.bind, config.server.grpc_port)
            .parse::<std::net::SocketAddr>()?;
        let grpc_state = jcode_grpc::LlmServerState::new(
            // 使用第一个可用 provider 作为默认
            state.providers.read().await.values().next().cloned()
                .unwrap_or_else(|| LlmProviderFactory::create_provider(
                    jcode_llm::config::LlmConfig::deepseek_chat()
                ))
        );
        // 注册所有已加载的 provider
        for (name, provider) in state.providers.read().await.iter() {
            grpc_state.register_provider(name.clone(), provider.clone());
        }
        let grpc_service = jcode_grpc::LlmServiceImpl::new(Arc::new(grpc_state));
        tokio::spawn(async move {
