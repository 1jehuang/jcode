...
use crate::enterprise::config::VirtualMemoryConfig;
...
impl VirtualMemoryManager {
    ...
    pub fn check_swap_status() -> SwapInfo {
        ...
        // Physical memory from mem_info (in KB, convert to GB)
        let total_physical_gb = mem_info
            .as_ref()
            .map(|m| m.total as f64 / 1024.0 / 1024.0)
            .unwrap_or(0.0);
        let available_physical_gb = mem_info
            .as_ref()
            .map(|m| m.avail as f64 / 1024.0 / 1024.0)
            .unwrap_or(0.0);

        // Swap memory - sys-info 0.9 doesn't have swap_info, use default 0
        // TODO: Implement swap detection using platform-specific APIs when needed
        let total_swap_gb = 0.0_f64;
        let available_swap_gb = 0.0_f64;
