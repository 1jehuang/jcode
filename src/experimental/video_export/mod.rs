...
text.emoji {{ font-family: "Noto Color Emoji", "Symbols Nerd Font", "{}", sans-serif; font-size: {:.1}px; dominant-baseline: text-before-edge; font-variant-ligatures: none; }}
    ...
    for y in 0..height {
        ...
        let mut x = 0u16;
        while x < width {
            if should_skip_cell(x) { x += 1; continue; }
            let cell = &buf[(x, y)];
            let bg = color_to_bg_hex(cell.bg);
            if bg == "#000000" { x += 1; continue; }
            let start_x = x;
            while x < width && !should_skip_cell(x) && color_to_bg_hex(buf[(x, y)].bg) == bg { x += 1; }
            svg.push_str(&format!(
        ...
        while x < width {
            ...
            let cell = &buf[(x, y)];
            let sym = cell.symbol();
            if sym == " " || sym.is_empty() { x += 1; continue; }
            if sym.contains('\x00') { x += 1; continue; }
            ...
            if needs_special_cell_render(sym) {
                ...
                svg.push_str(&render_special_text_cell(sym, x as u32 * cell_w, text_y, cell_w, &fg, bold));
                x += 1; continue;
            }
            ...
            if is_box_drawing(first_char) {
                let fg = color_to_hex(cell.fg);
                if first_char == '-' || first_char == 'â”? {
                    let start_x = x;
                    let thick = first_char == 'â”?;
                ...
                if let Some(fragment) = box_drawing_to_svg(first_char, x as u32 * cell_w, y as u32 * cell_h, cell_w, cell_h, &fg) {
            ...
            while x < width && !should_skip_cell(x) {
                let c = &buf[(x, y)];
                let s = c.symbol();
                if s.is_empty() || s.contains('\x00') { x += 1; continue; }
                let ch = s.chars().next().unwrap_or(' ');
                if is_box_drawing(ch) { break; }
                if color_to_hex(c.fg) != fg || c.modifier.contains(ratatui::style::Modifier::BOLD) != bold { break; }
                text_run.push_str(s); x += 1;
            }
            ...
            svg.push_str(&format!(
                ...
                start_x as u32 * cell_w, text_y, fg, font_weight, xml_escape(trimmed)
            ));
        }
    }
    ...
    for region in &mermaid_regions {
        if let Ok(png_data) = std::fs::read(&region.png_path) {
            ...
            let (draw_w, draw_h) = if region_w as f64 / region_h as f64 > aspect {
                let h = region_h; let w = (h as f64 * aspect) as u32; (w, h)
            } else {
                let w = region_w; let h = (w as f64 / aspect) as u32; (w, h)
            };
...
fn special_text_class(sym: &str) -> &'static str {
    if looks_like_emoji(sym) { "emoji" } else { "symbol" }
}
...
fn render_special_text_cell(
    ...
    format!(
        ...
        special_text_class(sym), x, y, fg, font_weight, text_len, xml_escape(sym)
    )
}
...
fn box_drawing_to_svg(
    ...
    let (left, right_seg, up, down, thick) = match ch {
        ...
        'â”? => (false, true, true, false, false),
        'â”? => (true, false, true, false, false),
        'â”? => (false, true, true, true, false),
        'â”? => (true, false, true, true, false),
        'â”? => (true, true, false, true, false),
        'â”? => (true, true, true, false, false),
        'â”? => (true, true, true, true, false),
        'â•? => {
            let r = cw.min(ch_h) / 2;
            return Some(format!(
        ...
        'â”? => (true, true, false, false, true),
        'â”? => (false, false, true, true, true),
        'â”? => (false, true, false, true, true),
        'â”? => (true, false, false, true, true),
        'â”? => (false, true, true, false, true),
        'â”? => (true, false, true, false, true),
        'â”? => (false, true, true, true, true),
        'â”? => (true, false, true, true, true),
        'â”? => (true, true, false, true, true),
        'â”? => (true, true, true, false, true),
        'â•? => (true, true, true, true, true),
        'â•? => {
            let g = 1u32;
            return Some(format!(
        ...
        'â–? => return Some(format!(r#"<rect x="{}" y="{}" width="{}" height="{}" fill="{}"/>"#, px, py, cw, ch_h, color)),
        'â–€' => return Some(format!(r#"<rect x="{}" y="{}" width="{}" height="{}" fill="{}"/>"#, px, py, cw, ch_h / 2, color)),
        'â–? => return Some(format!(r#"<rect x="{}" y="{}" width="{}" height="{}" fill="{}"/>"#, px, py+ch_h/2, cw, ch_h/2, color)),
        'â–? => return Some(format!(r#"<rect x="{}" y="{}" width="{}" height="{}" fill="{}"/>"#, px, py, cw/2, ch_h, color)),
        'â–? => return Some(format!(r#"<rect x="{}" y="{}" width="{}" height="{}" fill="{}"/>"#, px+cw/2, py, cw/2, ch_h, color)),
        'â–? | 'â–? | 'â–? => {
            let opacity = match ch { 'â–? => 0.25, 'â–? => 0.50, 'â–? => 0.75, _ => 0.5 };
    ...
    if left { svg.push_str(&format!(r#"<line x1="{}" y1="{}" x2="{}" y2="{}" stroke="{}" stroke-width="{}" stroke-linecap="round"/>"#, px, cy, cx, cy, color, stroke_w)); }
    if right_seg { svg.push_str(&format!(r#"<line x1="{}" y1="{}" x2="{}" y2="{}" stroke="{}" stroke-width="{}" stroke-linecap="round"/>"#, cx, cy, right, cy, color, stroke_w)); }
    if up { svg.push_str(&format!(r#"<line x1="{}" y1="{}" x2="{}" y2="{}" stroke="{}" stroke-width="{}" stroke-linecap="round"/>"#, cx, py, cx, cy, color, stroke_w)); }
    if down { svg.push_str(&format!(r#"<line x1="{}" y1="{}" x2="{}" y2="{}" stroke="{}" stroke-width="{}" stroke-linecap="round"/>"#, cx, cy, cx, b, color, stroke_w)); }
    Some(svg)
}
