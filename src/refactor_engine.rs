...
impl RefactorEngine {
    ...
    pub fn new(session_id: impl Into<String>) -> Self {
        ...
        Self {
            ...
            checkpoint: CheckpointManager::new(session_id, config.enable_checkpoints),
            config,
        }
    }

    /// 使用自定义配置创建
    pub fn with_config(session_id: impl Into<String>, config: RefactorConfig) -> Self {
        Self {
            coordinator: AtomicEditCoordinator::new(config.temp_dir.clone()),
            checkpoint: CheckpointManager::new(session_id, config.enable_checkpoints),
            config,
    ...
    pub async fn execute_edits(&mut self, ops: Vec<EditOperation>) -> RefactorResult {
        ...
        if self.config.enable_checkpoints {
            for op in &ops {
                if let Err(e) = self.checkpoint.track_edit(&op.file_path.to_string_lossy()).await {
                    warn!("Checkpoint backup failed for {:?}: {}", op.file_path, e);
                }
            }
    ...
    /// 使用 CheckpointManager 回滚到指定消息
    pub async fn rewind_to_message(&self, message_id: &str) -> Result<Vec<String>> {
        self.checkpoint.rewind(message_id).await
            .map_err(|e| anyhow::anyhow!("Rewind failed: {}", e))
    }

    /// 创建快照
    pub async fn create_snapshot(&self, message_id: &str) -> Result<()> {
        self.checkpoint.make_snapshot(message_id).await
            .map_err(|e| anyhow::anyhow!("Snapshot failed: {}", e))
    }

    /// 获取 CheckpointManager 的摘要
    pub async fn checkpoint_summary(&self) -> super::checkpoint::CheckpointSummary {
        self.checkpoint.summary().await
    }
}
